...
class TestLifecycle(AlertServiceTestCase):
    ...
    def test_manual_mock_alert_does_not_auto_resolve(self):
        svc = AlertService(enable_persistence=False, mode="mock")
        svc.mock_auto_resolve_sec = 10
        alert = svc.ingest(make_response(100), source="mock", run_id="MOCK_small_leak")
        svc._auto_resolve_due(now=alert["created_at"] + 11)
        self.assertEqual(svc.get(alert["alert_id"])["status"], "ACTIVE")
