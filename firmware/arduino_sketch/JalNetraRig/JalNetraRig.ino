...
// Increments once per publish attempt, connected or not, so a gap on the
// receiving end (USB bridge or MQTT subscriber) is evidence of a dropped
// packet rather than ambiguous silence.
...
void loop() {
    ...
    Serial.printf(
        "[TELEMETRY] seq=%lu Qin=%.3f Qout=%.3f Qbr=%.3f L/min | pulses(in/out/br)=%lu/%lu/%lu | "
