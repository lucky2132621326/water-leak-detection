...
public:
    ...
    // `seq` increments once per publish attempt (including ones dropped by a
    // disconnected client) so a gap on the receiving end is evidence of a lost
    // packet, not just "nothing arrived for a while."
    void publishTelemetry(uint32_t seq, double ts, bool clockSynced,
