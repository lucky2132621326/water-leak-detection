...
try {
  ...
  const writes = [
    ["/api/mode", { mode: "mock", scenario_id: "manual_control" }],
    ["/api/scenarios/run", { scenario_id: "normal_operation" }],
    ["/api/benchmark/evaluate", { run_id: "RUN_001" }],
    ["/api/leak/toggle", { action: "CLOSE" }],
    ["/api/mock/control/release", {}],
    ["/api/calibration", {}],
    ["/api/self-test", {}],
  ...
  const operatorMutation = await expectStatus(
    ...
    200,
    {
      ...
      body: JSON.stringify({ mode: "mock", scenario_id: "manual_control" }),
    },
  );
  const operatorPayload = await operatorMutation.json();
  if (!operatorPayload.success || operatorPayload.mode !== "mock") {
    throw new Error("Operator mode mutation did not succeed");
  }
} finally {
