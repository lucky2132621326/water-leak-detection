...
| **Backend connects but all collections are empty** | Fresh Docker volume; nothing recorded yet | `docker compose exec mongo mongosh jal_netra_mock --eval "db.telemetry.countDocuments()"` | Not a fault — open **Mock Scenarios** and run one. Direct POSTs require the configured `X-API-Key`. |
